  ...
  <div class="card p-6 rounded-xl shadow-sm bg-white">
    <form method="POST" action="{{ route('rooms.store') }}" class="space-y-4">
      ...
      <div>
        ...
        <div class="relative">
          <span class="absolute left-3 top-2 text-gray-600 font-medium text-sm">Rp</span>
          <input type="number" name="price_per_night" step="1" required value="{{ old('price_per_night') }}" class="w-full px-3 py-2 pl-9 border border-slate-200 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent {{ $errors->has('price_per_night') ? 'border-red-500' : '' }}" placeholder="100000">
        </div>
