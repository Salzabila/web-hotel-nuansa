    ...
    <form method="POST" action="{{ route('rooms.update', $room) }}" class="space-y-6">
      ...
      <div>
        ...
        <div class="relative">
          ...
          <input type="number" name="price" step="1" value="{{ $room->price }}" required class="w-full px-4 py-2 pl-10 border border-slate-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent {{ $errors->has('price') ? 'border-red-500' : '' }}">
        </div>
      ...
      <div class="flex gap-4 pt-4">
        ...
        <a href="{{ route('rooms.index') }}" class="flex-1 bg-slate-300 hover:bg-slate-400 text-gray-900 font-semibold py-3 rounded-lg transition-colors flex items-center justify-center gap-2">
